...
export const DeleteTransactionScene = new Scene(
    ...
    Scene.default(async (ctx) => {
        if (ctx.textQuery === BUTTON_QUERY.confirm) {
            ...
            const createdAt = formatTimestamp(transactionOptions.createdAt);
            ...
            if (result) {
                await ctx.replyWithMarkdownV2(
                    escapeMarkdownV2(`✅ Successfully deleted transaction «${formatAmount(transactionOptions.amount)} *${AccountCurrency[account.currency]}* (${createdAt})» for account: «${account.name}»`),
                );
                logger.info({userId: ctx.user.id, transactionOptions}, `Deleted transaction: ${transactionOptions._id!}`);
                return exit(ctx, true);
            } else {
                await ctx.replyWithMarkdownV2(
                    escapeMarkdownV2(`❌ Failed delete account: «${formatAmount(transactionOptions.amount)} *${AccountCurrency[account.currency]}* (${createdAt})» for account: «${account.name}»`),
                );
